...
const FeatureSection = ({ title, subtitle, description, items, imageUrl, direction="left"}: { title: string, subtitle: string, description: string, items: any, imageUrl: string, direction: string }) => {

  function leftrightimage(left: boolean){
    if(left){
      return (    
  ...
  return (
    <section className="relative block px-6 py-10 md:py-20 md:px-10">
      ...
      {leftrightimage(direction == "right")}
        <div className="sm:text mx-auto max-w-lg text-center md:w-1/2 md:text-left">
          ...
          <div className='h-6 flex w-full'></div>
        </div>
        {leftrightimage(direction == "left")}
      </div>
    </section>
  );
};
